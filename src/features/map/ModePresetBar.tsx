 ...
 * Icon-only mode buttons (dense segmented control, ~40px within the 48px bar);
 * the accessible name carries the full mode label. A failed mode is flagged on
 * its icon (the degraded state) via `failedMode`.
 *
...
export default function ModePresetBar({
  ...
  return (
    <div
      ...
      className="pointer-events-auto flex items-center justify-between gap-2 rounded-full border border-white/[.11] bg-[#0d110e]/92 p-1 shadow-[0_16px_44px_rgba(0,0,0,.34)] backdrop-blur-2xl"
    >
        ...
        {MODES.map((m) => {
          ...
          return (
            <button
              ...
              className={`relative grid size-10 place-items-center rounded-full transition-[background-color,color,box-shadow] ${
