...
describe("walkingIsochrone", () => {
  it("normalizes to rings sorted ascending by minutes, regardless of input order", async () => {
    providerFetch.mockResolvedValue(orsResponse([poly(2633), poly(861), poly(1744)]));
  ...
  it("sends [lng, lat] order and the CALIBRATED ranges in the request body", async () => {
    ...
    expect(JSON.parse(opts.init.body).range).toEqual([861, 1744, 2633]);
  });

  it("requests SLOW's own scaled ranges, not Normal's (the pace must reach ORS)", async () => {
    // Until task 064 every ORS contract test ran at the default pace, so a
    // broken pace→ranges path would have shipped green. 3 km/h ⇒ the anchor
    // triple × 50/80.
    providerFetch.mockResolvedValue(orsResponse([poly(517), poly(1046), poly(1580)]));
    const result = await walkingIsochrone(44.4268, 26.1025, "slow");
    const opts = providerFetch.mock.calls[0][1] as { init: { body: string } };
    expect(JSON.parse(opts.init.body).range).toEqual([517, 1046, 1580]);
    expect(result.rings.map((r) => r.minutes)).toEqual([15, 30, 45]);
  });

  it("caches Slow and Normal under DIFFERENT keys (a pace change must refetch)", async () => {
    providerFetch.mockResolvedValue(orsResponse([poly(861), poly(1744), poly(2633)]));
    await walkingIsochrone(44.4268, 26.1025, "normal");
    providerFetch.mockResolvedValue(orsResponse([poly(517), poly(1046), poly(1580)]));
    await walkingIsochrone(44.4268, 26.1025, "slow");
    expect(providerFetch).toHaveBeenCalledTimes(2);
    expect([...store.keys()].filter((k) => k.startsWith("iso:foot:v4:"))).toHaveLength(2);
  });
  ...
  it("throws when a requested range is missing (only 2 rings)", async () => {
    providerFetch.mockResolvedValue(orsResponse([poly(861), poly(1744)]));
  ...
  it("throws when a range is duplicated instead of the full 15/30/45 set", async () => {
    providerFetch.mockResolvedValue(orsResponse([poly(861), poly(861), poly(1744)]));
    await expect(walkingIsochrone(44.4, 26.1)).rejects.toThrow(/requested ranges/i);
  });
  ...
  it("serves a cache hit without a second fetch and returns the same value", async () => {
    ...
    // v4 key: pace-scoped; rings cached at the pre-064 walk speed must never
    // be served again (the geometry changed, the coordinates did not).
    expect([...store.keys()]).toEqual(["iso:foot:v4:normal:44.40000,26.10000"]);
  });
  ...
  it("coalesces two concurrent cold requests for the same origin into ONE fetch", async () => {
    ...
    resolveFetch(orsResponse([poly(861), poly(1744), poly(2633)]));
  ...
  it("rejects a well-typed ring whose coordinates are null/empty (never reaches the client)", async () => {
    const bad = { properties: { value: 861 }, geometry: { type: "Polygon", coordinates: null } };
    providerFetch.mockResolvedValue(orsResponse([bad, poly(1744), poly(2633)]));
    await expect(walkingIsochrone(44.4, 26.1)).rejects.toThrow(/invalid geometry/i);

    const empty = { properties: { value: 861 }, geometry: { type: "Polygon", coordinates: [] } };
    providerFetch.mockResolvedValue(orsResponse([empty, poly(1744), poly(2633)]));
  ...
  it("rejects garbage nested one level down ([null] / non-array members)", async () => {
    const nested = { properties: { value: 861 }, geometry: { type: "Polygon", coordinates: [null] } };
    providerFetch.mockResolvedValue(orsResponse([nested, poly(1744), poly(2633)]));
    await expect(walkingIsochrone(44.42, 26.12)).rejects.toThrow(/invalid geometry/i);

    const strings = { properties: { value: 861 }, geometry: { type: "Polygon", coordinates: ["x"] } };
    providerFetch.mockResolvedValue(orsResponse([strings, poly(1744), poly(2633)]));
...
describe("drivingIsochrone (car, tasks 053/058)", () => {
  // Car uses the driving-car profile and relabels to 10/20/30 min. Task 058
  // makes it TIME-AWARE: the nominal 600/1200/1800 s free-flow ranges are
  // DIVIDED by the traffic slot's congestion factor, so the request carries the
  // SCALED ranges. midday (×1.5) → [400,800,1200]; am-peak (×2.1) → smaller.
  // These assertions pin the driving request CONTRACT (profile URL, no Accept
  // header, scaled ranges, iso:car:v2:{frev}:est:{slotId} cache key).
  const midday = carTrafficSlot(3, 12); // factor 1.5
  const amPeak = carTrafficSlot(3, 8); // factor 2.1
  const MIDDAY_RANGES = scaledCarRangesS([600, 1200, 1800], 1.5); // [400,800,1200]

  it("requests the driving-car profile with the SCALED (traffic-adjusted) ranges, [lng,lat] order, and NO Accept header", async () => {
    ...
    expect(parsed.range).toEqual([400, 800, 1200]);
    expect(Object.keys(opts.init.headers)).not.toContain("Accept");
  });

  it("heavier traffic requests strictly smaller ranges (am-peak < midday)", async () => {
    providerFetch.mockResolvedValue(orsResponse(scaledCarRangesS([600, 1200, 1800], 2.1).map(poly)));
  ...
  it("relabels the driving rings to 10/20/30 min, ascending regardless of input order", async () => {
    providerFetch.mockResolvedValue(orsResponse([poly(1200), poly(400), poly(800)]));
    const result = await drivingIsochrone(44.4268, 26.1025, midday);
    expect(result.rings.map((r) => r.minutes)).toEqual([10, 20, 30]);
    expect(result.origin).toEqual({ lat: 44.4268, lng: 26.1025 });
  });

  it("caches under an isolated iso:car:v2:{frev}:est:{slotId} key (never collides with walk, or across slots)", async () => {
    providerFetch.mockResolvedValue(orsResponse(MIDDAY_RANGES.map(poly)));
    await drivingIsochrone(44.4, 26.1, midday);
    await drivingIsochrone(44.4, 26.1, midday);
    expect(providerFetch).toHaveBeenCalledTimes(1); // second served from cache
    expect([...store.keys()]).toEqual(["iso:car:v2:c1:est:midday:44.40000,26.10000"]);
    // A different slot must NOT hit the midday cache entry (frev/slot in key).
    providerFetch.mockResolvedValue(orsResponse(scaledCarRangesS([600, 1200, 1800], 2.1).map(poly)));
    await drivingIsochrone(44.4, 26.1, amPeak);
    expect(providerFetch).toHaveBeenCalledTimes(2);
    expect([...store.keys()]).toContain("iso:car:v2:c1:est:am-peak:44.40000,26.10000");
  });
