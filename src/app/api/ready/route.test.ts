...
describe("GET /api/ready (readiness — Railway healthcheck + Playwright gate)", () => {
  it("200 when the database is reachable and the provider config parses", async () => {
    probeDb.mockResolvedValue(true);
    const res = await GET();
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ready: true, db: true, config: true });
  });
  ...
  it("503 when the database is down — a broken deploy must report unhealthy", async () => {
    ...
    expect(await res.json()).toMatchObject({ ready: false, db: false, config: true });
  });
  ...
  it("503 when a provider env var is set-but-invalid (fails the healthcheck, not the first request) (task 007)", async () => {
    ...
    expect(body).toMatchObject({ ready: false, db: true, config: false });
    expect(body.configError).toMatch(/ORS_BASE_URL/);
  });
});
